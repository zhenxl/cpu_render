...
inline Real Schlick_approximation(Real eta) {
    return pow(eta - 1.0, 2.0) / pow(eta + 1.0, 2.0);
}

inline Spectrum fresnel_with_tint(Real specular_tint, Real specular, Real metallic, Spectrum base_color, Real eta, Real half_angle) {
    ...
    Spectrum K_s = (1.0 - specular_tint) + specular_tint * C_tint;
    Real R0 = Schlick_approximation(eta);
    auto C0 = specular * R0 * (1.0 - metallic) * K_s + metallic * base_color;
    auto F_m = C0 + (1.0 - C0) * pow((1 - half_angle), 5.0);
    return F_m;
}
...
inline Real Disney_GGX(Real anisotropic, Vector3d h, Real roughness, Frame frame) {
    Vector3d n_dot_h = to_local(frame, h);
    ...
    Real divend = pow((pow(n_dot_h.x, 2.0) / pow(alpha_x, 2.0) 
     + pow(n_dot_h.y, 2.0) / pow(alpha_y, 2.0) 
     + pow(n_dot_h.z, 2.0)), 2.0) * c_PI * alpha_x * alpha_y;
     return 1.0 / divend;

}

inline Real Disney_Smith(Real anisotropic, Vector3d w, Real roughness, Frame frame) {
    Real aspect = sqrt(1.0 - 0.9 * anisotropic);
    Real alpha_min = 0.0001;
    Real alpha_x = std::max(alpha_min, roughness*roughness / aspect);
    Real alpha_y = std::max(alpha_min, roughness * roughness * aspect);
    Vector3d w_l = to_local(frame, w);

    Real w_part = (pow(w_l.x * alpha_x, 2.0) + pow(w_l.y * alpha_y, 2.0)) / pow(w_l.z, 2.0);
    Real lambda_w = (sqrt(1 + w_part) - 1.0) / 2.0;
    Real G_w = 1.0 / (1.0 + lambda_w);
    return G_w;
}

inline Real normal_distribution(Real clearcoatGloss, Vector3d h, Frame frame) {
    Vector3d h_l = to_local(frame, h);
    Real alpha_g = (1 - clearcoatGloss) * 0.1 + clearcoatGloss * 0.001;
    Real alpha_g_square = alpha_g * alpha_g;
    Real divisor = log(alpha_g_square) * c_PI * (1 + (alpha_g_square - 1.0)* h_l.z * h_l.z);
    Real D_c = (alpha_g_square - 1.0) / divisor;
    return D_c;
}

inline Real Mask_shadowing(Vector3d w, Frame frame) {
    Vector3d w_l = to_local(frame, w);
    Real w_term = (pow((w_l.x * 0.25), 2.0) + pow((w_l.y * 0.25), 2.0)) / pow(w_l.z, 2.0);
    Real lambda_w = (sqrt(1 + w_term) - 1.0) / 2.0;
    return 1.0 / (lambda_w + 1.0);
}
