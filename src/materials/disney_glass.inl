...
Spectrum eval_op::operator()(const DisneyGlass &bsdf) const {
    ...
    Vector3d h = half_vector;
    Real out_half_angle = dot(h, dir_out);
    Real in_half_angle  = dot(h, dir_in);
    Real F_g = fresnel_dielectric(in_half_angle, eta);
    Real D_g =  Disney_GGX(anisotropic, h, roughness, frame);
    Real G_win  = Disney_Smith(anisotropic, dir_in, roughness, frame);
    Real G_wout = Disney_Smith(anisotropic, dir_out, roughness, frame);
    Real G_g = G_win * G_wout;
    Spectrum f_glass;
    if (reflect) {
        f_glass = (base_color * F_g  * D_g * G_g) / (4 * in_angle);
    } else {
        Real eta_factor = dir == TransportDirection::TO_LIGHT ? (1 / (eta * eta)) : 1;
        auto numerator   = sqrt(base_color) * (eta_factor * (1 - F_g) * D_g * G_g * eta * eta * fabs(out_half_angle * in_half_angle));
        auto denominator = in_angle * pow((in_half_angle + eta * out_half_angle) , 2.0);
        f_glass = numerator / denominator;
    }
    return f_glass;
}
...
Real pdf_sample_bsdf_op::operator()(const DisneyGlass &bsdf) const {
   ...
   Real eta = dot(vertex.geometric_normal, dir_in) > 0 ? bsdf.eta : 1 / bsdf.eta;
    ...
    Real D = Disney_GGX(anisotropic, half_vector, roughness, frame);
    Real G_in =  Disney_Smith(anisotropic, dir_in, roughness, frame);
    if (reflect) {
